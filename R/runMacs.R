...
runMacs = function(macs,species,nChr,segSites,popSize,split=NULL){
  ...
  if(is.null(split)){
    ...
    if(length(split)!=(length(popSize)-1)){
      stop("split must be of length popSize-1")
    }
    splitI
    splitJ
  }
  command = paste0(macs," ",sum(popSize)," ",speciesParams,splitI," ",speciesHist," -s ",sample.int(1e8,1)," 1>output.txt 2>/dev/null")
  currentDir = getwd()
  tmpDir = tempdir()
  setwd(tmpDir)
  output = list()
