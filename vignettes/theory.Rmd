...
This document describes the biological and statistical models implemented in AlphaSimR. The document consists of four sections: biological trait model, statistical trait model, meiosis model, and genomic selection models. The biological trait model section describes how an individual's genotype becomes a genetic value. The statistical trait model section describes the partitioning of a population's total genetic variance into variance components. The meiosis model section describes the method for simulating meiosis and genetic recombination. Finally, the genomic selection models section provides a brief overview for the genomic selection models available in AlphaSimR.
...
AlphaSimR deals with dominance in polyploids by sticking to the partially parameterized model given in equation (5). This model is equivalent to modeling digenic dominance with no higher order dominance terms. The rationale behind this approach is that a model with only digenic dominance will be a reasonable first-order approxiamation for partial dominance in highly polygenic traits.
...
The function for genotype-by-environment effects is given above in equation (8). The items on the right-hand side of this equation are an environmental covariate ($w$) times the sum of a constant value ($\mu_G$) and the summation over all QTL for the product of the genotype-by-environment effects ($g$) and the scaled additive dosage ($x_A$). This function does not correspond to a classic quantitative genetics models, so its design requires a bit of explanation.

The genotype-by-environment model used in AlphaSimR is inspired by Finlay and Wilkinson's regression approach for analyzing data for genotype-by-environment interaction [@FinlayWilkinson]. In its simplest form, the approach summarizes individuals with two values: an intercept representing their mean performance and a slope representing their linear response to the environmental mean. The genotype-by-environmental model in AlphaSimR assumes this precise model. The environmental mean and slope come from equation (8). The intercept comes from everything else in equation (1).

The environmental covariate ($w$) in equation (8) is an environmental factor and everything else is in the equation is a genetic factor. The environmental covariate does not represent the environmental mean. The environmental mean is actually a population specific value and it is controlled by both environmental and genetic factors. The best way to understand what the environmental covariate is, is to understand its distribution.

The environmental covariate is always sampled from a normal distribution with mean 0. The mean of this distribution represents the average environment or the target environment. This means that the value for the genotype-by-environment function is always 0 for the target environment, regardless of an individual's genotype. Except for phenotypes, AlphaSimR also always reports genetic values and variances for the target environment. This means that AlphaSimR does not include genotype-by-environment effects in these values and variances.

The variance of the environmental covariate depends on user supplied inputs.
