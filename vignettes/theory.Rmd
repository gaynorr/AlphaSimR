...
This document describes biological and statistical models implemented in AlphaSimR. The document consists of four sections: traits, variance components, meiosis, and genomic selection. The traits section describes how an individual's genotype becomes a genetic value which in turn becomes a phenotype. The variance components section describes the partitioning of a population's total genetic variance into variance components. The meiosis section describes how genetic recombination is modeled. Finally, the genomic selection section provides a brief overview of the genomic selection models available in AlphaSimR.

\pagebreak

# Traits

The biological models for traits in AlphaSimR are based on classic models used in quantitative genetics. This means anyone with introductory knowledge in quantitative genetics should recognize most aspects of AlphaSimR's models. This section will assume the reader is such a person and focus on aspects of AlphaSimR's models that may not be obvious or don't fit within a classic model. This section begins with an explaination of the system AlphaSimR uses to name and classify traits and continues with a more detailed explanation of the implementation.
...
Equation (1) can be slightly modified to represent all traits in the **ADEG** framework. The left-hand side of the equation consists of the following terms: $GV$, which represents an individual’s genetic value; $x$, which represents a vector of QTL genotype dosages; and $w$, which represents an environmental covariate. The right-hand side of the equation consists of an intercept ($\mu$) and four functions. The intercept is used set the user specified trait mean in a founder population. The functions corresponding to each of the biological effects. Any other trait in the **ADEG** framework will lack one or more of the biological effects in equation (1). These can be represented with a modified version of equation (1) that lacks the corresponding functions for those biological effects.
...
The function for epistatic effects is given above in equation (7). The summation in the right-hand side is over pairs of QTL, because AlphaSimR uses a simplified model of epistasis that restricts epistatic interactions to non-overlapping pair of loci. Since epistatic loci are not allowed to overlap, the number of pairs is equal to half the number of QTL and each QTL only occurs in one pair. The remaining elements in the right-hand side of equation (7) are epistatic effects ($e$), the scaled additive dosage for the first locus in a pair ($x_{A_1}$) and the scaled additive dosage for the second locus in a pair ($x_{A_2}$).
...
AlphaSimR users do not directly observe nor set the value of the environmental covariate. Instead, they are responsible for indirectly setting its value by providing a randomly sampled p-value for each environment. AlphaSimR then uses this p-value to calculate the appropriate value of $w$ for any level of environmental variance. Users should note that p-values follow a uniform distribution over the range of zero to one, so p-values should be randomly generated using R's built-in `runif` function. A p-value of 0.5 always results in $w$ equaling zero and is equivalent to the target environment. A p-value of 0.5 is also the default value used in AlphaSimR functions, so if the user fails to provide a p-value they will receive a phenotype for the target environment.
...
This section describes how AlphaSimR partitions a population’s total genetic variance into variance components. It will be assumed that the reader already has some familiarity with the concept of variance components. If not, the reader should first read an introduction to the topic which can be found any quantitative genetics textbook. The focus of this section is limited to the calculation of the variance components and the topic of their interpretation will not be discussed. For more information on interpreting variance components, the reader is again advised to consult a quantitative genetics textbook.

The formulas provided in most textbooks for calculating variance components are not applicable to AlphaSimR. This is because those formulas often depend on assumptions that are not met in all simulations. Furthermore, AlphaSimR simulates a wide range of ploidy levels, but most equations only apply to diploids. 

The solution to this problem is quite simple. AlphaSimR simply calculates variance components by directly applying the basic principles used to derive the textbook formulas. It’s likely that most readers aren’t very familiar with these principles, because introductory quantitative genetics courses spend little if any time explaining how the formulas are derived. Thus, the majority this section will be dedicated to outlining how these principles are used to perform the appropriate calculations. 

## Calculation 

The goal is to decompose a population’s total genetic variance ($V_G$) into additive variance ($V_A$), (digenic) dominance variance ($V_D$) and additive-by-additive epistatic variance ($V_{AA}$). Additional variance components occur in real populations, but only these three are considered because they represent all sources of genetic variation \footnote{Genotype-by-environment interactions are excluded from variance component calculations (See section 2.5.3)} modeled by the traits in AlphaSimR. The decomposition of total variance into variance components must be orthogonal (statistically independent) for a population in linkage equilibrium. This means that covariance between variance components will be zero and that total genetic variance will equal the sum of the variance components. However, nearly all populations will have some linkage disequilibrium, so covariance may not be zero. The full formula for the relationship between total variance and the variance components is thus given in the equation below.
...
The variances (and covariances) in equation (11) come from decomposing genetic values and taking the variance of the resulting components. This decomposition 
...
### Additive-by-additive Epistatic Deviations (AA)
...
\begin{equation}
y = X \beta + Z_f a_f + Z_m a_m + (h \mu_d + W d^*) + e
\end{equation}
