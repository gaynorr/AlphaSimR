...
The function for genotype-by-environment effects is given above in equation (9). The right-hand side of the equation contains two parts: an environmental covariate ($w$) and a genotype specific slope. The genotype specific slope includes all terms within the parenthesis on the right-hand side. These terms are a constant value ($\mu_G$) and a summation over all QTL for the product of a genotype-by-environment effect ($g$) and the scaled additive dosage ($x_A$). 

The following paragraphs explain how equation (9) models genotype-by-environment interactions. The approach taken to do this is to first explain the pieces of the model and then explain how those pieces come together. This involves first describing the sampling procedure for each variable in equation (9) and then explaining how those variables interact with each other to form a coherent model for genotype-by-environment interactions.





Note that this summation is equivalent to the summation used for additive effects, so genotype specific slope is itself effectively an additive trait.


Unlike the previously discussed effects, this function does not from a classic quantitative genetics model. 

AlphaSimR genotype-by-environment effects model is designed as a biological representation of Finlay-Wilkinson regression. Finlay-Wilkinson regression is a classic method for analyzing genotype-by-environment interactions [@FinlayWilkinson]. 

Finlay-Wilkinson regression involves regressing a genotype's environment specific performance onto an environmental mean to estimate a slope and an intercept for that genotype. The intercept represents the genotype's overall mean and the slope represents the genotype's linear response to the environmental mean. Lack-of-fit in the regression can indicate non-linear response, but since AlphaSimR only models linear genotype-by-environment effects this can be ignored here.

Similarly to Finlay-Wilkinson regression, AlphaSimR models an individual's genotype-by-environment interaction with an intercept and a slope. The intercept represents the individual's performance in the target environment and the slope represents the individual's responsiveness to an environmental covariate. The intercept comes from all terms in equation (1), except for the genotype-by-environment effects function ($G(x, w)$). The slope and the environmental covariate are modeled by the genotype-by-environment effects function.

The right-hand side of equation (8) is the product of an environmental covariate ($w$) and an individual's genotype-by-environment slope. The genotype-by-environment slope is contained within parentheses, and it is the sum of a constant value ($\mu_G$) and the summation over all QTL for the product of the genotype-by-environment effect ($g$) and the scaled additive dosage ($x_A$).
