...
The following paragraphs will explain how the above equations model genotype-by-environment interactions. Initially, the explanation will cover a case where a population has genotype-by-environment interaction variance, but no environmental variance. This case is represented by the default arguments in AlphaSimR. Later, how the parameters are changed to model both genotype-by-environment interaction variance and environmental variance will be described.
...
The genotype specific slope in equation (9) represents the genetic component of the genotype-by-environment interaction. An astute reader will notice that this equation is very similar to the equation for an additive trait. Indeed, the genotype specific slope is really just an additive trait. The term $\mu_G$ serves a similar role to $\mu$ in equation (1) and the rest summation on the right-hand side of equation (9) is similar to the summation used for additive effects, right-hand side of equation (4). Just like for an additive trait, the values of the genotype specific slope are scaled to achieve a specific mean and variance in a founder population. In this case, the mean is set to zero and the variance is set to the user specified genotype-by-environment interaction variance.
...
### Including Environmental Variance

The previous description is limited to a founder population with genotype-by-environment interaction variance and no environmental variance. 

### Equivalency to Other Models

### User Interaction

AlphaSimR users do not directly observe nor set the value of the environmental covariate. Instead, they are responsible for indirectly setting its value by providing a randomly sampled p-value for each environment. AlphaSimR then uses this p-value to calculate the appropriate value of $w$ for any level of environmental variance. Users should note that p-values follow a uniform distribution over the range of zero to one, so p-values should be randomly generated using R's built-in `runif` function. A p-value of 0.5 always results in $w$ equalling zero and is equivalent to the target environment. A p-value of 0.5 is also the default value used in AlphaSimR functions, so if the user fails to provide a p-value they will recieve a phenotype for the target environment.

Genetic values and genetic variances reported by AlphaSimR are always for the target environment. Since the target environment corresponds to $w$ equalling zero, equation (8) is effectively ignored in these calculations. Only the values for phenotypes and phenotypic variance will ever reflect a contribution from genotype-by-environment interaction.
