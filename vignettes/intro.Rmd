...
The code above uses the `quickHaplo` function to generate the initial haplotypes. The `quickHaplo` function generates haplotypes by randomly sampling 1s and 0s. This approach is equivalent to modeling a population in linkage and Hardy-Weinberg equillibrium with allele frequencies of 0.5. This approach is very rapid, but does not generate realistic haplotypes. This makes the approach great for protyping code, but ill suited for some types a simulations.
