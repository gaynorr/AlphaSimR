...
test_that("setPhenoPop", {
  ...
  for (i in seq_along(multiPop@pops)) {
    for (t in seq_len(SP$nTraits)) {
      expect_equal(
        unname(phenoMed[i, t]),
        median(multiPop@pops[[i]]@pheno[, t])
      )
    }
  }
  ...
  vec_fun <- function(ph) {
    as.numeric(colMeans(ph))
  } # returns numeric vector
  phenoVec <- setPhenoPop(
    multiPop,
    onlyPhenoPop = TRUE,
    FUN = vec_fun,
    simParam = SP
