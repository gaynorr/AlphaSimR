...
test_that("MultiPop assignment and replace methods", {
  ...
  expect_identical(mpA[["Group1"]], pop[1:2])

  # Append elements to a MultiPop using `[<-` and defined names
  mpB = newEmptyMultiPop()
  mpB[c('Group1','Group2')] = list(pop[1:2], pop[3:4])
  mpC = newEmptyMultiPop()
  mpC[c('Group1','Group2')] = newMultiPop(pop[1:2], pop[3:4])
  expect_identical(names(mpC), c("Group1", "Group2"))
  expect_identical(mpC["Group1"], newMultiPop(Group1 = pop[1:2]))
  
  # Append elements to a MultiPop using `$<-` and defined names
  mpD = newEmptyMultiPop()

  mpD$Group1 = pop[1:2]
  mpD$Group2 = pop[3:4]
  expect_identical(names(mpD), c("Group1", "Group2"))
  expect_identical(mpD$Group2, pop[3:4])
  
  # Test that the MultiPops created with different methods are identical
  expect_identical(mpA, mpB)
  expect_identical(mpB, mpC)
  expect_identical(mpC, mpD)

  # Replace elements in a MultiPop using `[[<-`, `[<-`, and `$<-`
  mpA[["Group1"]] = pop[5:6]
  mpA[[2]] = pop[7:8]
  mpB[c('Group1')] = list(pop[5:6])
  mpB[2] = newMultiPop(pop[7:8])
  expect_identical(mpA, mpB)
  expect_identical(mpA[[1]], pop[5:6])
  expect_identical(mpA[["Group2"]], pop[7:8])
  expect_identical(mpB["Group1"], newMultiPop(Group1 = pop[5:6]))
  mpC$Group1 = pop[5:6]
  mpC[2:3] = list(pop[7:8], pop[9:10])
  expect_identical(mpC$Group1, pop[5:6])
  expect_identical(names(mpC), c("Group1", "Group2", ""))
  mpB[2:3] = newMultiPop(Group2 = pop[7:8], pop[9:10])
  expect_identical(mpB, mpC)
  ...
  mpD = newMultiPop(pop[1:2], pop[3:4], pop[5:6], pop[7:8], pop[9:10])
  ...
  mp2 = newMultiPop(pop1 = pop[1:2], pop2 = pop[3:4],
                    mpA = newMultiPop(pop3 = pop[5:6],
                                      mpB = newMultiPop(pop4 = pop[7:8], pop5 = pop[9:10])))
  mp2copy = mp2
  ...
  expect_identical(mp2[["mpA"]][['mpB']][['pop5']], pop[9:10])
  ...
  expect_identical(mp2[['mpA']][['mpB']]['pop6'], newMultiPop(pop6 = pop[1:2]))
  expect_identical(mp2[['mpA']][['mpB']][['pop6']], pop[1:2])
  mp2[['mpA']][['mpB']]['pop6'] = NULL
  expect_length(mp2[['mpA']][['mpB']], 2)

  mp2[['mpA']][['mpB']][[3]] = mp2["pop6"]
  expect_identical(mp2[['mpA']][['mpB']][[3]], newMultiPop(pop6 = pop[1:2]))
  mp2[['mpA']][['mpB']][[3]] = NULL
  expect_length(mp2[['mpA']][['mpB']], 2)
  ...
  expect_error(
    dollar_set(mp, quote(c("A", "B")), pop[4:5]),
    "$ requires a single name",
    fixed = TRUE
  )
...
test_that("MultiPop show", {
  ...
  mp = newMultiPop(
    top1 = pop[1:4],
    top2 = newEmptyPop(simParam = SP),
    top3 = newMultiPop(
      mid1 = pop[5],
      mid2 = newMultiPop(pop[6],
                         pop[7:10])
    )
  )
  out1 = capture.output(show(mp))
  expect_true(any(grepl("Level 1:", out1, fixed = TRUE)))
  expect_true(any(grepl('An object of class "Pop" with 4 individual\\(s\\)', out1)))
  expect_true(any(grepl('An object of class "MultiPop" with 2 item\\(s\\)', out1)))
  expect_true(any(grepl('An object of class "Pop" with 1 individual\\(s\\)', out1)))
...
test_that("Multipop unnameMultiPop", {
  founderPop = quickHaplo(nInd = 1, nChr = 1, segSites = 1)
  SP = SimParam$new(founderPop)
  SP$nThreads = 1L
  SP$addTraitA(1)
  pop = newPop(founderPop, simParam = SP)

  mp2 = newMultiPop(pop1 = pop, pop2 = pop,
                    mpA = newMultiPop(pop3 = pop,
                                      mpB = newMultiPop(pop4 = pop, pop5 = pop)))
