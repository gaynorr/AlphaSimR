...
A breeding program based on the "historical breeding" portion of the Jenko et al. simulation will be used as an example [-@Jenko15]. This example simulation models 20 discrete generations of selection in a simplified animal breeding program. The discrete generations consist of 1000 animals, of which 500 are male and 500 are female. In each generation, the best 25 males are selected on the basis of their genetic value for a single polygenic trait and mated to the females to produce 1000 replacement animals. The quantitative trait is modeled as being controlled by 10,000 QTL. These QTL are equally split across 10 chromosome groups so that there are 1,000 QTL per chromosome. The mean genetic value of all individuals in a generation is recorded to construct a plot for the genetic gain per generation.
...
The code above uses the `runMacs` function. The `runMacs` function is a wrapper for the MaCS software, a coalescent simulation program included within the distribution of AlphaSimR [@MaCS]. The MaCS software is used by AlphaSimR to simulate bi-allelic genome sequences according to a population demographic history. In the above code no population demographic history is explicitly declared, so `runMacs` uses the default “GENERIC” history. The “GENERIC” history is the same history used by Jenko et al. to model cattle [-@Jenko15]. The `runMacs` function allows the user to specify one of several other predefined histories or supply their own history. The available histories are listed in the `runMacs` help document.

The `runMacs` function is not the only way to create founder haplotypes in AlphaSimR. There are actually a few functions, but the most likely second choice is `newMapPop`. This function imports user supplied haplotypes; allowing the user to use their own means for generating haplotypes or import real marker data. 
...
The final line of code defines how gender is determined in the simulation. Gender will be systematically assigned (i.e. male, female, male, ...). Systematic assignment is used to ensure that there is always equal numbers of males and females.
...
The final lines of code are for modeling 20 generations of selection and mating. AlphaSimR has a host of functions for modeling both selection and mating. In this example the `selectCross` function is used, because it efficiently combines both selection and mating in a single function call. To model multiple generation of selection, the function call is placed in a loop with a line of code for tracking the population mean. Using a loop makes code easier to read and decreases avoids needless duplication of code. In this loop “pop” is overwritten in each generation. This is done because it is more efficient from a memory usage perspective and there is no need to retain older populations in this simulation. The code for the loop is given below.
